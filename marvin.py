...
def set_servo_pulse(channel, pulse):
    ...
    off = int((pulse / 20000.0) * 4096)
    pca.channels[channel].duty_cycle = off

def ramp_servo(channel, target, duration_ms, steps=20):
...
def move_servo(channel, position, speed=500, time=1000):
    if channel in REVERSE_CHANNELS and REVERSE_CHANNELS[channel]:
        target = 3000 - position
    else:
        target = position
    target = max(1000, min(2000, target))
    threading.Thread(target=ramp_servo, args=(channel, target, time)).start()
